...
<main>
   <div class="container-fluid mb-5">
      <section>
         <div class="col-md-13">
            <div class="card">
               <div class="tulisan-putih card-body">
                  @if($title == 'searchbyyear')
                  <h4>Report List Search By Year</h4>
                  @elseif($title == 'searchbymonth')
                  <h4>Report List Search By Month</h4>
                  @elseif($title == 'searchbydate')
                  <h4>Report List Search By Date</h4>
                  @endif
                  <h5 class="tulisan-putih card-body-title">
                     @if($title == 'todayorder')
                     Report List Today Orders
                     @elseif($title == 'todaydeliv')
                     Report List Today Delivered
                     @elseif($title == 'mountdeliv')
                     Report List This Mount Delivered
                     @elseif($title == 'searchbyyear')
                     <span class="badge badge-success">Total Amount on {{ $subtitle }} is @rupiah($total)</span>
                     @elseif($title == 'searchbymonth')
                     <span class="badge badge-success">Total Amount on {{ $subtitle }} is @rupiah($total)</span>
                     @elseif($title == 'searchbydate')
                     <span class="badge badge-success">Total Amount on {{ $subtitle }} is @rupiah($total)</span>
